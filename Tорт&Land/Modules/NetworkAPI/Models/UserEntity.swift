...
extension UserEntity {
    init(from model: Cake_User) {
        ...
        self = UserEntity(
            ...
            mail: model.mail,
            imageURL: model.hasImageURL ? model.imageURL.value.replaceLocalhost() : nil,
            headerImageURL: model.hasHeaderImageURL ? model.headerImageURL.value.replaceLocalhost() : nil
        )
    }
    ...
    public init(from model: ProfileEntity) {
        self = UserEntity(
            ...
            headerImageURL: model.headerImageURL?.replaceLocalhost()
        )
    }
}
